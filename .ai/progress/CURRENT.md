...
Last updated: 2026-09-06 (Vunexo Vault added — Project #3, spec through implementation, then a same-day session 2 adding terminal polish and an optional GUI companion).
...
Plain Rust binary crate, `apps/vunexo-vault/` — the first non-Tauri project in this monorepo. Binary name `vunexo`. Version `0.1.0` — not yet released, not yet run on real Windows/Linux hardware (smoke-tested on macOS, see below). **CLI remains the primary interface** — an optional desktop companion, [Vunexo Vault GUI](../../apps/vunexo-vault-gui/), was added session 2 (2026-09-06) as a second interface to the identical shared crate, not a replacement.

**Committed and pushed** (session 2, 2026-09-06): `17aecbc` on `vunexolabs/vunexo-tools` `main` (CLI + docs + CI + release workflow — the GUI companion, added later the same session, is not yet committed). GitHub's push protection blocked the first attempt — a test fixture (a fake-but-realistic-looking Stripe key, used to prove the scanner catches Stripe-shaped secrets) tripped GitHub's own secret scanner. Fixed by swapping it for an obviously-fake, low-entropy placeholder (`sk_test_FAKEFAKEFAKEFAKE1234`) in all three places it appeared (`pattern_scanner.rs`, `cli_integration.rs`, `crypto-and-scanning-engine.md`), re-verified all tests, then amended and re-pushed. Author `Abdul Hakeem <abdul.hakeem5764@gmail.com>`, no Claude co-author trailer; `gh auth` switched to the `vunexolabs` account before pushing.
...
Backend: **54 tests passing** (38 unit + 16 integration — was 52/36+16 before session 2 added `HookInstaller::is_installed()` + 2 tests for the GUI's Hooks screen), `cargo fmt`/`clippy` clean, zero warnings. Verified independently (not just trusting the implementing agent's own report): re-ran the full `build && test && fmt --check && clippy` sequence myself, and read `infrastructure/age_vault_store.rs` and `infrastructure/hook_installer.rs` in full to confirm the crypto path uses only `age::scrypt::Recipient`/`Identity` (no Vunexo-chosen KDF/salt/work-factor anywhere) and the hook logic matches the locked "never clobber a foreign hook" behavior.

**Terminal output polish (session 2)**: colorized `secrets scan` (yellow rule name, dimmed redacted preview, a green/yellow clean-or-N-findings summary line), `env list`'s active-environment highlight, and green checkmarks on `init`/`hooks install`/`hooks uninstall` success messages — via `owo-colors`' `if_supports_color`, which respects `NO_COLOR` and non-TTY output automatically (no manual detection code needed). Purely decorative — no exit codes or output *content* changed, except one new "no findings, clean" / "N finding(s)" summary line on `secrets scan`, which required updating one existing test (`scan_exits_4_when_a_finding_is_present_and_0_otherwise`) that had asserted a clean scan's stdout was fully empty.

**Shared `[lib]` target (session 2)**: `apps/vunexo-vault/src/lib.rs` now exposes `domain`/`application`/`infrastructure` (plus `pub use secrecy;` so a consumer can name `SecretString` without a second direct dependency) — `main.rs` consumes them via `use vunexo_vault::{...}` instead of its own `mod` declarations. Pure refactor, confirmed by the full test suite passing unchanged before and after. This exists specifically so [Vunexo Vault GUI](../../apps/vunexo-vault-gui/) can depend on the identical crypto/domain code via a path dependency rather than a second implementation — see `.ai/product-vunexo-vault.md`'s 2026-09-06 amendment.
...
- `.github/workflows/ci.yml` has a `vault` job (cross-platform matrix, `cargo build/test/fmt/clippy`) plus, since session 2, `frontend-vault-gui`/`backend-vault-gui` jobs for the GUI companion (the latter includes the Tauri Linux system-dependency step the CLI's own job doesn't need). Not yet run against a push including the GUI (the CLI-only push already ran once at `17aecbc`, before the GUI existed).
- `.github/workflows/release-vunexo-vault.yml` (tag prefix `vault-v*`) — a plain-Rust build+package+`softprops/action-gh-release` workflow, not `tauri-action` (the CLI has no Tauri bundle). `.github/workflows/release-vunexo-vault-gui.yml` (new, session 2, tag prefix `vault-gui-v*`) — a real `tauri-action` workflow, matching `release-expense-manager.yml`'s shape, since the GUI *does* produce a real bundle. Neither has ever been triggered.
- Root `README.md` lists Vault as Project #3 and links the GUI as its companion (not a separate numbered project). Both apps' own READMEs cross-link each other.
- The CLI + docs + CI + CLI-release-workflow are committed and pushed (`17aecbc`). The GUI companion (`apps/vunexo-vault-gui/`, `docs/vunexo-vault/gui-ux.md`, the new CI jobs, the GUI release workflow, and the three small shared-crate additions) is built, tested, and manually verified but **not yet committed** as of this entry.

## Vunexo Vault GUI (`apps/vunexo-vault-gui/`, added session 2, 2026-09-06)

Optional Tauri+React desktop companion — a second interface to the exact same `vunexo-vault` crate, added after the user asked for "a UI... neatly organized" and, when asked to clarify, chose "both — nicer terminal output, plus an optional companion GUI, CLI stays primary." Since this contradicted the locked "no GUI" architecture principle, it went through a full plan-mode round before building: `.ai/product-vunexo-vault.md` carries an explicit, dated amendment (not a silent rewrite) permitting a GUI *only* as a second interface to the shared crate, never a reimplementation.

**Screens** (`docs/vunexo-vault/gui-ux.md`, the locked screen inventory): Open Project (folder picker + recent projects), Unlock (environment + passphrase), Secrets (masked list, reveal/copy/add/edit/remove), Environments (list/create/switch), Scan (run + findings table), Hooks (install/uninstall), Settings (vault version, project path, Lock). Deliberately no "run a command with secrets" feature in the GUI — that stays CLI-only, so there's exactly one place secrets ever reach a child process's environment.

**Architecture**: `apps/vunexo-vault/src/lib.rs` (new) exposes `domain`/`application`/`infrastructure`; the GUI's `src-tauri/Cargo.toml` depends on it via a plain path dependency — no `age`/`secrecy`/`regex` as direct GUI dependencies, everything crypto-related comes through the shared crate. This is a deliberate, disclosed departure from Billing/Expense Manager's precedent of zero cross-app Rust code sharing, justified because the CLI and GUI here are two interfaces to the *same* vault format and the *same* crypto, not two independent products — reimplementing `age` encrypt/decrypt a second time would risk the two copies silently drifting apart on a security-critical path.

**Three small, disclosed shared-crate changes** (additive only, reviewed and accepted): `lib.rs` gained `pub use secrecy;` (so the GUI can name `SecretString` without a second dependency — a pure visibility change, no new crypto surface); `HookInstaller` gained a read-only `is_installed()` query (the CLI never needed this, being fire-and-forget, but the Hooks screen needs to render current state before offering an action) with 2 new unit tests.

**Visual design**: Expense Manager's token-based Tailwind system (CSS variables, `.card`/`.table-base`/`.badge`) as the base, with Billing's most recent polish layered on top (rounded-card table containers, pill-style row actions, a project-initial sidebar badge, a left accent bar on the active nav item) — matching what the user specifically referenced when asking for the GUI.

**Verification — independently re-checked, not just the implementing agent's own report**: `cargo build/test/fmt/clippy` clean for both `apps/vunexo-vault` (54 tests, confirming the shared-lib extraction didn't break the CLI) and `apps/vunexo-vault-gui/src-tauri`; `pnpm typecheck/lint/build` clean for the GUI frontend (one benign `react-refresh` warning, same class as `useCurrency.tsx`/`useTheme.tsx`'s pre-existing ones elsewhere in the repo). Read `commands.rs`, `state.rs`, and `error.rs` in full: confirmed every command is a thin wrapper with no reimplemented crypto/scanning, the passphrase lives only in a session-scoped `Mutex`, and `unlock` verifies the passphrase against the real vault file *before* committing an environment switch (a real ordering bug the implementing agent caught and fixed during its own review, confirmed correct by reading the code).

**A real manual click-through, not a disclosed gap** (macOS Accessibility/AppleScript UI scripting, same technique used for Billing session 14 — no screenshot capability in this sandbox): launched `pnpm tauri dev`, opened the exact project folder used for the CLI's own smoke test (a real `.age` vault created earlier via the CLI), unlocked it with the real passphrase, confirmed the Secrets screen listed `API_KEY` and revealing it showed the exact same value the CLI had shown, planted a fresh fake secret and confirmed Scan caught it with the same redacted preview format, and confirmed the Hooks screen correctly showed "Not installed" (matching the real git-hook state left by earlier CLI testing in that same directory). This proves both interfaces genuinely read and write the identical encrypted vault, not just that each independently claims to.

## History (agreed order)

1. ~~Project started: spec through implementation, all in one session~~ — done 2026-09-06 (session 1). Round 1 (product spec) went through three rounds of user review/correction before locking — see `2026-09-06.md` for the exact wording changes (the `age`-identity boundary, the locked command surface/pillars/tagline, the plaintext-never-persisted invariant). Rounds 2–6 (user flows, storage schema, application architecture, CLI UX, crypto & scanning engine) written directly; Round 7 (implementation) delegated to a background agent against all five locked docs; Round 8 (testing) done as part of the same implementation pass, all Round 6 test vectors passing; Round 9 (release readiness) done directly (CI job, release workflow, README, root README). Committed and pushed as `17aecbc` (session 2, after fixing a GitHub push-protection block on a too-realistic fake Stripe key test vector).
2. ~~Terminal output polish + optional GUI companion~~ — done 2026-09-06 (session 2). Colorized CLI output; `.ai/product-vunexo-vault.md` amended to permit an optional GUI; `apps/vunexo-vault-gui/` built via a background agent against a locked `gui-ux.md`, independently re-verified (tests, code review, and a real manual click-through). Not yet committed as of this entry.

## Next up

- Decide when to commit/push the GUI companion (currently built, tested, and verified locally only).
- Decide when (if) to actually run either release workflow and cut real builds; confirm Windows/Linux behavior once CI runs for the first time on a push that includes the `vault`/`vault-gui` jobs.
- Consider whether a `--json` CLI output mode, an OS-keychain-backed passphrase option, or CI/CD integrations are worth a genuine V2 — all deliberately out of V1 scope per the locked spec's guardrail against scope creep toward a team/cloud platform.

## Verification commands

```bash
# CLI, from apps/vunexo-vault/
cargo build && cargo test --quiet && cargo fmt --check && cargo clippy --all-targets --quiet

# GUI backend, from apps/vunexo-vault-gui/src-tauri/
cargo build && cargo test --quiet && cargo fmt --check && cargo clippy --all-targets --quiet

# GUI frontend, from apps/vunexo-vault-gui/
pnpm typecheck && pnpm lint && pnpm build
```
...
- `2026-09-06.md` — Vunexo Vault (Project #3) pitched, scoped, designed, and built end to end in session 1: a CLI secrets manager + git-hook leak scanner, the first plain-Rust project in the monorepo. Round 1 locked across three user-review passes; Rounds 2–6 written directly; Round 7 implementation delegated to a background agent; 52 tests passing, a real manual smoke test (including an actual git commit genuinely blocked and then genuinely succeeding) run against the compiled binary; committed and pushed. Session 2 (same day): terminal output polish, plus an optional desktop GUI companion (a disclosed amendment to the locked "no GUI" boundary) sharing the CLI's exact crypto/domain code via a new `[lib]` target — independently verified via code review and a real macOS-accessibility-scripted click-through proving both interfaces read/write the identical vault.
